...
public:
	...
	*/
	...
	/*
	Hàm kiểm tra một điểm có nằm trong một ma trận hay không
	Tham số
	- x, y: vị trí điểm
	- matrix: ma trận kiểm tra
	Trả về:
	 - 0: Nếu điểm đó không nằm trong ma trận
	 - 1: Nếu điểm đó nằm trong ma trận
	*/
	bool IsInside(const int& x, const int& y, const Mat& matrix);
	
	/*
	Hàm tính kích thước toàn vẹn của một hình chữ nhật đã được quay
	Tham số
	- oldHeight, oldWidth: kích thước cũ
	- newHeight, newWidth: kích thước mới
	- angle: góc quay
	*/
	void CalcSizeRotatedRect(
		const int& oldHeight, 
		const int& oldWidth, 
		int& newHeight, 
		int& newWidth,
